...
def split_data(input_data, output_data):
    training_sequence=input_data[:int(sequence_matrix_for_enbedding.shape[0]*0.8)]
    training_output=output_data[:int(sequence_matrix_for_enbedding.shape[0]*0.8)]
    testing_sequence=input_data[int(sequence_matrix_for_enbedding.shape[0]*0.8):]
    testing_output=output_data[int(sequence_matrix_for_enbedding.shape[0]*0.8):]
...
def LSTM_model_on_one_hot_sequence(sequence_length, self_loss, learning_rate=0.01):
    model = Sequential()
    # model.add(Embedding(5, 1, input_length=1285))
    model.add(Dense(512, kernel_initializer='uniform', input_shape=(1,5*sequence_length,)))
    ...
    model.compile(loss=self_loss, optimizer=sgd)
    return model
...
def model_report(LSTM_model, testing_sequence, testing_output, batch_size_value=50):
    LSTM_model.summary()
    score = model.evaluate(testing_sequence, testing_output, batch_size=batch_size_value, verbose=2)
    return score
...
def caparison_figure(predict_data, testing_output):
    ...
    plt.scatter(index,predicted_output,label='predict output')
    plt.legend()