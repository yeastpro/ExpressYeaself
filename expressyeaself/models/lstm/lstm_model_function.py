...
def read_length(encoded_sequences):
    """
    this function helps to read the sequence length and the data amount
    """
    data_length=int(encoded_sequences.shape[0])
    sequence_length=int(encoded_sequences.shape[1])
    return data_length, sequence_length

# def matrix_reshape_to_array(matrix, data_amount, sequence_length):
#     """
#     this function helps to reshape the sequences matric
#     """
...
def split_data(input_data, output_data):
    ...
    return training_sequence, training_output, testing_sequence, testing_output
...
def scale_output(raw_data, data_length):
    """
    the function to scale the output
    """
#     output_data=[]
...
#         output_data.append(output)
    output_data=np.array(raw_data).reshape(data_length, 1, 1)
    return output_data

def input_of_one_hot_sequence(raw_matrix, data_amount, sequence_length):
    """
    the function to reshape the matrix to the input format of the lstm
    """
...
def lstm_model_on_one_hot_sequence(sequence_length, training_sequence, training_output, testing_sequence, testing_output, self_loss='mean_squared_error',learning_rate=0.01, epochs_value=20, batch_size_value=50):
    """
    ...
    model.add(Dense(1024, kernel_initializer='uniform', input_shape=(1,5*sequence_length,)))
    ...
    model.fit(training_sequence, training_output, epochs=epochs_value, batch_size=batch_size_value, verbose=1)
    score = model.evaluate(testing_sequence, testing_output)
    predicted_output_raw=model.predict(testing_sequence)
    predicted_output=predicted_output_raw.reshape(len(predicted_output_raw),1)
    
    return score, predicted_output

# def LSTM_model_training(LSTM_model, training_sequence, training_output, epochs_value=20, batch_size_value=50):
#     return LSTM_model.fit(training_sequence, training_output, epochs=epochs_value, batch_size=batch_size_value, verbose=1)

# def model_report(LSTM_model, testing_sequence, testing_output, batch_size_value=50):
# #     LSTM_model.summary()
#     score = model.evaluate(testing_sequence, testing_output)
#     return score
...
def caparison_figure(predict_data, testing_output):
    """
    the function to plot the comparison figures
    """
...
def lstm_training(data_path,self_loss='mean_squared_error',learning_rate=0.01, epochs_value=2, batch_size_value=50):
    """
    the wrapping function
    """
    sequence_list, raw_output, max_value = encode.encode_sequences_with_method(data_path, 'One-Hot')
    data_number, bp_number=read_length(sequence_list)
    reshaped_sequence_matrix=input_of_one_hot_sequence(sequence_list, data_number, bp_number)
    output=scale_output(raw_output, data_number)
    train_x, train_y, test_x, text_y=split_data(reshaped_sequence_matrix, output)
    score, predict_data=lstm_model_on_one_hot_sequence(bp_number, train_x, train_y, test_x, text_y, self_loss=self_loss,learning_rate=learning_rate, epochs_value=epochs_value, batch_size_value=batch_size_value)
#     LSTM_model_training(LSTM_model, train_x, train_y, epochs_value=2, batch_size_value=50)
#     socre=model_report(LSTM_model, testing_sequence, testing_output, batch_size_value=50)
#     predicted_output=predict_data(LSTM_model, test_x)
    cp_figure=caparison_figure(predict_data, text_y)
