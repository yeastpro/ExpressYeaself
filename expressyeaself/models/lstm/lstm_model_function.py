...
def input_of_one_hot_sequence(raw_matrix, data_amount, sequence_length):
    """
    the function to reshape the matrix to the input
    format of the lstm
    """
    sequence_matrix = raw_matrix.reshape(data_amount, -1)
    one_hot_sequence_matrix = sequence_matrix.reshape(
        data_amount, 1, sequence_length * 5)
    return one_hot_sequence_matrix


def lstm_model_on_one_hot_sequence(
        sequence_length,
        training_sequence,
        training_output,
        testing_sequence,
    ...
    predicted_output = predicted_output_raw.reshape(
        len(predicted_output_raw), 1)
...
def lstm_training(
    ...
    """
    sequence_list, raw_output, max_value = encode.encode_sequences_with_method(
        data_path, 'One-Hot')
    data_number, bp_number = read_length(sequence_list)
    reshaped_sequence_matrix = input_of_one_hot_sequence(
        sequence_list, data_number, bp_number)
    output = scale_output(raw_output, data_number)
    train_x, train_y, test_x, text_y = split_data(
        reshaped_sequence_matrix, output)
    score, predict_data = lstm_model_on_one_hot_sequence(
        bp_number, train_x, train_y, test_x, text_y,
        self_loss=self_loss, learning_rate=learning_rate,
        epochs_value=epochs_value, batch_size_value=batch_size_value)
#     LSTM_model_training(LSTM_model, train_x, train_y,
#         epochs_value=2, batch_size_value=50)
#     socre=model_report(LSTM_model, testing_sequence,
#         testing_output, batch_size_value=50)
